...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Line'), ['action' => 'edit', $line->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Line'), ['action' => 'delete', $line->id], ['confirm' => __('Are you sure you want to delete # {0}?', $line->id)]) ?> </li>
...
<div class="lines view large-9 medium-8 columns content">
    <h3><?= h($line->name) ?></h3>
    <table class="vertical-table">
        ...
        <tr>
            ...
            <td><?= $line->delete ? __('Yes') : __('No'); ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Description') ?></h4>
        <?= $this->Text->autoParagraph(h($line->description)); ?>
    </div>
    <div class="related">
        <h4><?= __('Related Samples') ?></h4>
        <?php if (!empty($line->samples)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($line->samples as $samples): ?>
            <tr>
                <td><?= h($samples->id) ?></td>
